function ieee_rts_73_n3_110_search(ID)
...
tic
ps = case_ieee_rts_73;
k = 3; % k is the number of failed lines (n-k)
...
ps.bus(:,C.bu.Pd) = ps.bus(:,C.bu.Pd)*1.1;
ps.bus(:,C.bu.Qd) = ps.bus(:,C.bu.Qd)*1.1;
...
n_iters = size(Initial_Lines,1)/85;
Initial_Lines = Initial_Lines(n_iters*ID-n_iters+1:n_iters*ID,:);
    ...
    toc
    save(['ieee_rts_73_n-',num2str(k),'_110_search_',num2str(ID),'_of_85.mat'],'blackout','relay_count','MW_lost','p_out','buses_count','Initial_Lines');
end

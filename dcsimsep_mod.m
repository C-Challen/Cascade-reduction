function [is_blackout,relay_outages,MW_lost,p_out,busessep,movie_data] = dcsimsep_mod(ps,br_outages,bus_outages,opt,Intervention,B,Delay,Graph,TestType)  
% usage: [is_blackout,relay_outages,MW_lost,p_out,busessep,movie_data] = dcsimsep(ps,br_outages,bus_outages,opt,intervention)    
...
[sub_grids,n_sub_old] = findSubGraphs_mod(ps,C,Graph);
...
ps = dcpf_mod(ps,[],false,opt.verbose,B,Graph); % this one should not need to do any redispatch, just line flow calcs
% Get the power flow
...
br_out_new = [];
while t < t_max
    if Intervened == 0 % Not yet intervened
        if TestType == 2 % Wait for cascade to start
            if ~isempty(br_out_new) % At least one branch has failed
                if t2 == inf % Timer has not yet started   
                    t2 = t + Delay; % Start timer for intervention
                end
            end
        elseif TestType == 1 % Don't wait for cascade to start
            if t2 == inf % Timer has not yet started   
                t2 = t + Delay; % Start timer for intervention
            end
        end  
        if t >= t2 % If timer has finished
            ps.branch(Intervention,C.br.status) = 0; % Intervene
            Intervened = 1; % Set flag
        end
    end
    %% Step 3. Find sub-grids in the network and check for major separation

    [sep,sub_grids,n_sub,p_out,busessep] = check_separation(ps,opt.sim.stop_threshold,opt.verbose,Graph);
%    if t >= 0 && Responded ~= 1
%        ps.branch(Triggered,C.br.status) = 0;
%        Responded = 1;
%    end
...
%         if opt.debug && any( Pg<Pg_min | Pg>Pg_max ), error('Pg is out of bounds'); end
    ...
    ps = dcpf_mod(ps,sub_grids,true,opt.verbose,B);
    if opt.debug
        % Check that
        ge_status = ps.gen(:,C.ge.status);
        Pg_max = ps.gen(:,C.ge.Pmax).*ge_status + EPS;
...
%    [ps.relay,br_out_new,dt,n_over] = update_relays(ps,opt.verbose,dt_max);
    [ps.relay,br_out_new,dt] = update_relays_mod(ps,opt.verbose,dt_max);
